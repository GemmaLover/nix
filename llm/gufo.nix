...
let
  ...
  gufoInstall = pkgs.writeShellApplication {
    ...
    text = ''
      ...
      # Создаём папку gufo внутри ~/llm/models.
      # Все модели этого движка изолированы здесь.
      mkdir -p "$MODELS_DIR"
...
in
{
  # === Регистрация скриптов в пользовательском окружении ===
  home.packages = [
    gufoInstall
    gufoStart
    gufoStop
